...
export default function UnifiedItineraryCanvas() {
  ...
  const finalTotal = p.total_price || 50000;
  const pricePerPerson = p.price_per_person || 25000;

  return (
    <div className="min-h-screen bg-background text-on-background flex flex-col font-body">
      {/* ─── Top Canvas Toolbar Bar ────────────────────────────────────────────── */}
      <header className="sticky top-0 z-40 bg-surface-container-high/90 backdrop-blur-md border-b border-outline-variant px-4 py-3 flex flex-wrap items-center justify-between gap-4">
        ...
        {/* Center: Quick Intake, 96-Template Selector & Visibility Mode */}
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
        {/* LEFT PANE: Unified Interactive Editor (7 cols) */}
        <div className="lg:col-span-6 xl:col-span-6 p-6 border-r border-outline-variant overflow-y-auto space-y-6 max-h-[calc(100vh-61px)]">
        ...
        {/* RIGHT PANE: Dual Live View Panel (5 cols) */}
        <div className="lg:col-span-6 xl:col-span-6 p-6 bg-surface-container overflow-y-auto max-h-[calc(100vh-61px)]">
          <div className="h-full bg-surface border border-outline-variant rounded-2xl shadow-xl overflow-hidden flex flex-col">
            {/* Live View Bar Header */}
            <div className="px-4 py-2.5 bg-surface-container-high border-b border-outline-variant flex items-center justify-between text-xs">
              <span className="font-bold text-on-surface-variant uppercase tracking-wider flex items-center gap-1.5">
                <span className="material-symbols-outlined text-[16px] text-primary">description</span>
                Layout Preview: <strong>{activeTemplateSlug.toUpperCase()}</strong>
              </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-emerald-400 font-bold px-2 py-0.5 bg-emerald-500/10 rounded-full">
                  Live Dynamic Sync
                </span>
              </div>
            </div>
